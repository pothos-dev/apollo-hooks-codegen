...
function plugin(schema, documents, config) {
    ...
    return join.apply(void 0, [disclaimer,
        imports,
        ''].concat(documents.map(function (documentFile) { return formatDocumentFile(documentFile); }), ['',
        formatInputNames(),
        boilerplate]));
    function formatInputNames() {
        findRecursiveInputReferences(Object.keys(inputTypeNames));
        if (Object.keys(inputTypeNames).length == 0)
            return '';
        return join.apply(void 0, ['/*',
            ' * GraphQL InputTypes',
            ' */',
            ''].concat(Object.keys(inputTypeNames).map(function (name) {
            var node = schema.getType(name);
            return formatGraphQLInputObjectType(node);
        })));
    }
    function findRecursiveInputReferences(newNames) {
        if (newNames.length == 0)
            return;
        var newNames2 = [];
        for (var _i = 0, newNames_1 = newNames; _i < newNames_1.length; _i++) {
            var name_1 = newNames_1[_i];
            var node = schema.getType(name_1);
    ...
    function formatDocumentFile(file) {
        return join('', '', '/*', ' * Operations from ' + file.filePath, ' */', '', formatDocumentNode(file.content));
    }
    function formatDocumentNode(node) {
        return node.definitions
            .map(function (definitionNode) { return formatDefinition(definitionNode); })
            .join('\n');
    }
    function formatDefinition(node) {
        switch (node.kind) {
    ...
    function formatOperationDefinition(node) {
        return ('export const ' +
            node.name.value +
            ' = ' +
            ...
            ',' +
            formatSelectionSet(node.selectionSet, selectSchemaObject(node.operation)) +
            '>(gql`\n' +
    ...
    function formatVariableDefinitions(nodes) {
        var list = nodes.map(function (variableNode) {
            return formatVariableDefinition(variableNode);
        });
    ...
    function formatNameTypeNode(node) {
        switch (node.name.value) {
            ...
            default:
                var nodeType = schema.getType(node.name.value).astNode.kind;
    ...
    function formatCustomScalar(name) {
        var customName = config.scalarTypes && config.scalarTypes[name];
    ...
    function selectSchemaObject(operation) {
        switch (operation) {
            case 'query':
